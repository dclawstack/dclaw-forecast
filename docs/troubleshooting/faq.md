...
### How do I update DClaw Forecast?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp forecast --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Forecast without Kubernetes?

Yes, for local development:

```bash
cd dclaw-forecast/frontend && npm run dev
cd dclaw-forecast/backend && uvicorn dclaw_forecast.main:app --reload
```
