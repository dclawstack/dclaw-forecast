...
# Check events
kubectl get events -n dclaw-forecast --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-forecast deployment/dclaw-forecast-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-forecast-db-credentials -n dclaw-forecast
   ```
